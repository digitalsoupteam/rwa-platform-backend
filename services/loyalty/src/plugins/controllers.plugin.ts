...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const getFeesCtrl = withTraceSync('loyalty.init.controllers.get_fees', () => getFeesController(servicesPlugin));

  const getReferralsCtrl = withTraceSync('loyalty.init.controllers.get_referrals', () =>
    getReferralsController(servicesPlugin),
  );

  const registerReferralCtrl = withTraceSync('loyalty.init.controllers.register_referral', () =>
    registerReferralController(servicesPlugin),
  );

  const getReferrerWithdrawsCtrl = withTraceSync('loyalty.init.controllers.get_referrer_withdraws', () =>
    getReferrerWithdrawsController(servicesPlugin),
  );

  const getReferrerClaimHistoryCtrl = withTraceSync('loyalty.init.controllers.get_referrer_claim_history', () =>
    getReferrerClaimHistoryController(servicesPlugin),
  );

  const getCommissionHistoryCtrl = withTraceSync('loyalty.init.controllers.get_commission_history', () =>
    getCommissionHistoryController(servicesPlugin),
  );

  const createReferrerWithdrawTaskCtrl = withTraceSync('loyalty.init.controllers.create_referrer_withdraw_task', () =>
    createReferrerWithdrawTaskController(servicesPlugin),
  );

  const plugin = withTraceSync('loyalty.init.controllers.plugin', () =>
