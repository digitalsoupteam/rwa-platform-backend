...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const createGalleryCtrl = withTraceSync('gallery.init.controllers.create_gallery', () =>
    createGalleryController(servicesPlugin),
  );

  const updateGalleryCtrl = withTraceSync('gallery.init.controllers.update_gallery', () =>
    updateGalleryController(servicesPlugin),
  );

  const deleteGalleryCtrl = withTraceSync('gallery.init.controllers.delete_gallery', () =>
    deleteGalleryController(servicesPlugin),
  );

  const getGalleryCtrl = withTraceSync('gallery.init.controllers.get_gallery', () =>
    getGalleryController(servicesPlugin),
  );

  const getGalleriesCtrl = withTraceSync('gallery.init.controllers.get_galleries', () =>
    getGalleriesController(servicesPlugin),
  );

  const createImageCtrl = withTraceSync('gallery.init.controllers.create_image', () =>
    createImageController(servicesPlugin),
  );

  const updateImageCtrl = withTraceSync('gallery.init.controllers.update_image', () =>
    updateImageController(servicesPlugin),
  );

  const deleteImageCtrl = withTraceSync('gallery.init.controllers.delete_image', () =>
    deleteImageController(servicesPlugin),
  );

  const getImageCtrl = withTraceSync('gallery.init.controllers.get_image', () => getImageController(servicesPlugin));

  const getImagesCtrl = withTraceSync('gallery.init.controllers.get_images', () => getImagesController(servicesPlugin));

  const plugin = withTraceSync('gallery.init.controllers.plugin', () =>
