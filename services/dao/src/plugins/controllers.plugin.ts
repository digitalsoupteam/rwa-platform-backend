...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const getTimelockTasksCtrl = withTraceSync('dao.init.controllers.get_timelock_tasks', () =>
    getTimelockTasksController(servicesPlugin),
  );

  const getProposalsCtrl = withTraceSync('dao.init.controllers.get_proposals', () =>
    getProposalsController(servicesPlugin),
  );

  const getStakingCtrl = withTraceSync('dao.init.controllers.get_staking', () => getStakingController(servicesPlugin));

  const getTreasuryWithdrawalsCtrl = withTraceSync('dao.init.controllers.get_treasury_withdrawals', () =>
    getTreasuryWithdrawalsController(servicesPlugin),
  );

  const getStakingHistoryCtrl = withTraceSync('dao.init.controllers.get_staking_history', () =>
    getStakingHistoryController(servicesPlugin),
  );

  const getVotesCtrl = withTraceSync('dao.init.controllers.get_votes', () => getVotesController(servicesPlugin));

  const plugin = withTraceSync('dao.init.controllers.plugin', () =>
