...
export const createClientsPlugin = async (
  ...
  await withTraceAsync('signers-manager.init.clients.rabbitmq_connect', async () => await rabbitMQClient.connect());

  const signerClient = withTraceSync('signers-manager.init.clients.signer', () => new SignerClient(rabbitMQClient));

  await withTraceAsync('signers-manager.init.clients.signer_initialize', async () => await signerClient.initialize());

  const plugin = withTraceSync('signers-manager.init.clients.plugin', () =>
    new Elysia({ name: 'Clients' })
      .decorate('rabbitMQClient', rabbitMQClient)
      .decorate('signerClient', signerClient)
      .onStop(async () => {
        await withTraceAsync('signers-manager.stop.clients', async () => {
