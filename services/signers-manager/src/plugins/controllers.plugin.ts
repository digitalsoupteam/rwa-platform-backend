...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const createSignatureTaskCtrl = withTraceSync('signers-manager.init.controllers.create_signature_task', () =>
    createSignatureTaskController(servicesPlugin),
  );

  const getSignatureTaskCtrl = withTraceSync('signers-manager.init.controllers.get_signature_task', () =>
    getSignatureTaskController(servicesPlugin),
  );

  const plugin = withTraceSync('signers-manager.init.controllers.plugin', () =>
    new Elysia({ name: 'Controllers' }).use(createSignatureTaskCtrl).use(getSignatureTaskCtrl),
  );
