...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const createCompanyCtrl = withTraceSync('company.init.controllers.create_company', () =>
    createCompanyController(servicesPlugin),
  );

  const updateCompanyCtrl = withTraceSync('company.init.controllers.update_company', () =>
    updateCompanyController(servicesPlugin),
  );

  const deleteCompanyCtrl = withTraceSync('company.init.controllers.delete_company', () =>
    deleteCompanyController(servicesPlugin),
  );

  const getCompanyCtrl = withTraceSync('company.init.controllers.get_company', () =>
    getCompanyController(servicesPlugin),
  );

  const getCompaniesCtrl = withTraceSync('company.init.controllers.get_companies', () =>
    getCompaniesController(servicesPlugin),
  );

  const addMemberCtrl = withTraceSync('company.init.controllers.add_member', () => addMemberController(servicesPlugin));

  const removeMemberCtrl = withTraceSync('company.init.controllers.remove_member', () =>
    removeMemberController(servicesPlugin),
  );

  const grantPermissionCtrl = withTraceSync('company.init.controllers.grant_permission', () =>
    grantPermissionController(servicesPlugin),
  );

  const revokePermissionCtrl = withTraceSync('company.init.controllers.revoke_permission', () =>
    revokePermissionController(servicesPlugin),
  );

  const plugin = withTraceSync('company.init.controllers.plugin', () =>
