...
export const createServicesPlugin = (repositoriesPlugin: RepositoriesPlugin) => {
  const blogsService = withTraceSync(
    ...
    () => new BlogsService(repositoriesPlugin.decorator.blogRepository, repositoriesPlugin.decorator.postRepository),
  );

  const plugin = withTraceSync('blog.init.services.plugin', () =>
    new Elysia({ name: 'Services' }).use(repositoriesPlugin).decorate('blogsService', blogsService),
  );
