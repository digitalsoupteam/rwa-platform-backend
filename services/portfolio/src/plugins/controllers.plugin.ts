...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const getBalancesCtrl = withTraceSync('portfolio.init.controllers.get_balances', () =>
    getBalancesController(servicesPlugin),
  );

  const getTransactionsCtrl = withTraceSync('portfolio.init.controllers.get_transactions', () =>
    getTransactionsController(servicesPlugin),
  );

  const plugin = withTraceSync('portfolio.init.controllers.plugin', () =>
    new Elysia({ name: 'Controllers' }).use(getBalancesCtrl).use(getTransactionsCtrl),
  );
