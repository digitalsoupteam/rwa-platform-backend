...
export const createClientsPlugin = async (providerUrl: string, walletPrivateKey: string) => {
  ...
  await withTraceAsync('testnet-faucet.init.clients.blockchain_initialize', async () => {
  ...
  const plugin = withTraceSync('testnet-faucet.init.clients.plugin', () =>
    new Elysia({ name: 'Clients' }).decorate('blockchainClient', blockchainClient).onStop(async () => {
      await withTraceAsync('testnet-faucet.stop.clients', async () => {
