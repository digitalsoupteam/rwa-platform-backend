...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const getHistoryCtrl = withTraceSync('testnet-faucet.init.controllers.get_history', () =>
    getHistoryController(servicesPlugin),
  );

  const getUnlockTimeCtrl = withTraceSync('testnet-faucet.init.controllers.get_unlock_time', () =>
    getUnlockTimeController(servicesPlugin),
  );

  const requestGasCtrl = withTraceSync('testnet-faucet.init.controllers.request_gas', () =>
    requestGasController(servicesPlugin),
  );

  const requestHoldCtrl = withTraceSync('testnet-faucet.init.controllers.request_hold', () =>
    requestHoldController(servicesPlugin),
  );

  const requestPlatformCtrl = withTraceSync('testnet-faucet.init.controllers.request_platform', () =>
    requestPlatformController(servicesPlugin),
  );

  const plugin = withTraceSync('testnet-faucet.init.controllers.plugin', () =>
