...
export async function createApp(port: number, mongoUri: string) {
  const repositoriesPlugin = await withTraceAsync(
    'example.init.repositories_plugin',
    async () => await createRepositoriesPlugin(mongoUri),
  );

  const servicesPlugin = withTraceSync('example.init.services_plugin', () => createServicesPlugin(repositoriesPlugin));

  const controllersPlugin = withTraceSync('example.init.controllers_plugin', () =>
    createControllersPlugin(servicesPlugin),
  );

  const app = withTraceSync('example.init.elysia', (ctx) => {
