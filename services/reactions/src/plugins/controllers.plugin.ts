...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const setReactionCtrl = withTraceSync('reactions.init.controllers.set_reaction', () =>
    setReactionController(servicesPlugin),
  );

  const resetReactionCtrl = withTraceSync('reactions.init.controllers.reset_reaction', () =>
    resetReactionController(servicesPlugin),
  );

  const getEntityReactionsCtrl = withTraceSync('reactions.init.controllers.get_entity_reactions', () =>
    getEntityReactionsController(servicesPlugin),
  );

  const getReactionsCtrl = withTraceSync('reactions.init.controllers.get_reactions', () =>
    getReactionsController(servicesPlugin),
  );

  const plugin = withTraceSync('reactions.init.controllers.plugin', () =>
