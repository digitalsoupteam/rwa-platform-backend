...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const createFolderCtrl = withTraceSync('documents.init.controllers.create_folder', () =>
    createFolderController(servicesPlugin),
  );

  const updateFolderCtrl = withTraceSync('documents.init.controllers.update_folder', () =>
    updateFolderController(servicesPlugin),
  );

  const deleteFolderCtrl = withTraceSync('documents.init.controllers.delete_folder', () =>
    deleteFolderController(servicesPlugin),
  );

  const getFolderCtrl = withTraceSync('documents.init.controllers.get_folder', () =>
    getFolderController(servicesPlugin),
  );

  const getFoldersCtrl = withTraceSync('documents.init.controllers.get_folders', () =>
    getFoldersController(servicesPlugin),
  );

  const createDocumentCtrl = withTraceSync('documents.init.controllers.create_document', () =>
    createDocumentController(servicesPlugin),
  );

  const updateDocumentCtrl = withTraceSync('documents.init.controllers.update_document', () =>
    updateDocumentController(servicesPlugin),
  );

  const deleteDocumentCtrl = withTraceSync('documents.init.controllers.delete_document', () =>
    deleteDocumentController(servicesPlugin),
  );

  const getDocumentCtrl = withTraceSync('documents.init.controllers.get_document', () =>
    getDocumentController(servicesPlugin),
  );

  const getDocumentsCtrl = withTraceSync('documents.init.controllers.get_documents', () =>
    getDocumentsController(servicesPlugin),
  );

  const plugin = withTraceSync('documents.init.controllers.plugin', () =>
