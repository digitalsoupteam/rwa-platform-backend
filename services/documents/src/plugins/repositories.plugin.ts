...
export const createRepositoriesPlugin = async (mongoUri: string) => {
  ...
  const documentRepository = withTraceSync('documents.init.repositories.document', () => new DocumentRepository());

  await withTraceAsync('documents.init.repositories_plugin.mongoose', async (ctx) => {
  ...
  const plugin = withTraceSync('documents.init.repositories.plugin', () =>
    new Elysia({ name: 'Repositories' })
      ...
      .onStop(async () => {
        await withTraceAsync('documents.stop.repositories_plugin', async () => {
