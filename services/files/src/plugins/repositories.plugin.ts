...
export const createRepositoriesPlugin = async (mongoUri: string) => {
  const fileRepository = withTraceSync('files.init.repositories.file', () => new FileRepository());

  await withTraceAsync('files.init.repositories_plugin.mongoose', async (ctx) => {
