...
export const createServicesPlugin = (repositoriesPlugin: RepositoriesPlugin, clientsPlugin: ClientsPlugin) => {
  const fileService = withTraceSync(
    ...
    () => new FileService(repositoriesPlugin.decorator.fileRepository, clientsPlugin.decorator.storageClient),
  );

  const plugin = withTraceSync('files.init.services.plugin', () =>
    new Elysia({ name: 'Services' }).use(repositoriesPlugin).use(clientsPlugin).decorate('fileService', fileService),
  );
