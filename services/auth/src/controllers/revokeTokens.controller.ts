...
import { revokeTokensRequest, revokeTokensResponse } from '../models/validation/user.validation';
import type { ServicesPlugin } from '../plugins/services.plugin';

export const createRevokeTokensController = (servicesPlugin: ServicesPlugin) => {
  return new Elysia({ name: 'RevokeTokensController' }).use(servicesPlugin).post(
    '/revokeTokens',
