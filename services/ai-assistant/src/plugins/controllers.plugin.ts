...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const createAssistantCtrl = withTraceSync('ai-assistant.init.controllers.create_assistant', () =>
    createAssistantController(servicesPlugin),
  );

  const updateAssistantCtrl = withTraceSync('ai-assistant.init.controllers.update_assistant', () =>
    updateAssistantController(servicesPlugin),
  );

  const deleteAssistantCtrl = withTraceSync('ai-assistant.init.controllers.delete_assistant', () =>
    deleteAssistantController(servicesPlugin),
  );

  const getAssistantCtrl = withTraceSync('ai-assistant.init.controllers.get_assistant', () =>
    getAssistantController(servicesPlugin),
  );

  const getUserAssistantsCtrl = withTraceSync('ai-assistant.init.controllers.get_user_assistants', () =>
    getUserAssistantsController(servicesPlugin),
  );

  const createMessageCtrl = withTraceSync('ai-assistant.init.controllers.create_message', () =>
    createMessageController(servicesPlugin),
  );

  const getMessageHistoryCtrl = withTraceSync('ai-assistant.init.controllers.get_message_history', () =>
    getMessageHistoryController(servicesPlugin),
  );

  const getMessageCtrl = withTraceSync('ai-assistant.init.controllers.get_message', () =>
    getMessageController(servicesPlugin),
  );

  const deleteMessageCtrl = withTraceSync('ai-assistant.init.controllers.delete_message', () =>
    deleteMessageController(servicesPlugin),
  );

  const updateMessageCtrl = withTraceSync('ai-assistant.init.controllers.update_message', () =>
    updateMessageController(servicesPlugin),
  );

  const plugin = withTraceSync('ai-assistant.init.controllers.plugin', () =>
