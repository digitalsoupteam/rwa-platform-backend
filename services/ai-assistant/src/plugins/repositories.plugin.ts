...
export const createRepositoriesPlugin = async (mongoUri: string) => {
  ...
  const messageRepository = withTraceSync('ai-assistant.init.repositories.message', () => new MessageRepository());

  await withTraceAsync('ai-assistant.init.repositories_plugin.mongoose', async (ctx) => {
  ...
  const plugin = withTraceSync('ai-assistant.init.repositories.plugin', () =>
    new Elysia({ name: 'Repositories' })
      .decorate('assistantRepository', assistantRepository)
      .decorate('messageRepository', messageRepository)
      .onStop(async () => {
        await withTraceAsync('ai-assistant.stop.repositories_plugin', async () => {
