...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const getRawPriceDataCtrl = withTraceSync('charts.init.controllers.get_raw_price_data', () =>
    getRawPriceDataController(servicesPlugin),
  );

  const getOhlcPriceDataCtrl = withTraceSync('charts.init.controllers.get_ohlc_price_data', () =>
    getOhlcPriceDataController(servicesPlugin),
  );

  const getPoolTransactionsCtrl = withTraceSync('charts.init.controllers.get_pool_transactions', () =>
    getPoolTransactionsController(servicesPlugin),
  );

  const getVolumeDataCtrl = withTraceSync('charts.init.controllers.get_volume_data', () =>
    getVolumeDataController(servicesPlugin),
  );

  const plugin = withTraceSync('charts.init.controllers.plugin', () =>
