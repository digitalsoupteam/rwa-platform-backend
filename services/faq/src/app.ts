...
export async function createApp(port: number, mongoUri: string) {
  const repositoriesPlugin = await withTraceAsync(
    'faq.init.repositories_plugin',
    async () => await createRepositoriesPlugin(mongoUri),
  );

  const servicesPlugin = withTraceSync('faq.init.services_plugin', () => createServicesPlugin(repositoriesPlugin));

  const controllersPlugin = withTraceSync('faq.init.controllers_plugin', () => createControllersPlugin(servicesPlugin));

  const app = withTraceSync('faq.init.elysia', (ctx) => {
