...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const createTopicCtrl = withTraceSync('faq.init.controllers.create_topic', () =>
    createTopicController(servicesPlugin),
  );

  const updateTopicCtrl = withTraceSync('faq.init.controllers.update_topic', () =>
    updateTopicController(servicesPlugin),
  );

  const deleteTopicCtrl = withTraceSync('faq.init.controllers.delete_topic', () =>
    deleteTopicController(servicesPlugin),
  );

  const getTopicCtrl = withTraceSync('faq.init.controllers.get_topic', () => getTopicController(servicesPlugin));

  const getTopicsCtrl = withTraceSync('faq.init.controllers.get_topics', () => getTopicsController(servicesPlugin));

  const createAnswerCtrl = withTraceSync('faq.init.controllers.create_answer', () =>
    createAnswerController(servicesPlugin),
  );

  const updateAnswerCtrl = withTraceSync('faq.init.controllers.update_answer', () =>
    updateAnswerController(servicesPlugin),
  );

  const deleteAnswerCtrl = withTraceSync('faq.init.controllers.delete_answer', () =>
    deleteAnswerController(servicesPlugin),
  );

  const getAnswerCtrl = withTraceSync('faq.init.controllers.get_answer', () => getAnswerController(servicesPlugin));

  const getAnswersCtrl = withTraceSync('faq.init.controllers.get_answers', () => getAnswersController(servicesPlugin));

  const plugin = withTraceSync('faq.init.controllers.plugin', () =>
