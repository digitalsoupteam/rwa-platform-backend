...
export const createServicesPlugin = (repositoriesPlugin: RepositoriesPlugin) => {
  const faqService = withTraceSync(
    ...
    () => new FaqService(repositoriesPlugin.decorator.topicRepository, repositoriesPlugin.decorator.answerRepository),
  );

  const plugin = withTraceSync('faq.init.services.plugin', () =>
    new Elysia({ name: 'Services' }).use(repositoriesPlugin).decorate('faqService', faqService),
  );
