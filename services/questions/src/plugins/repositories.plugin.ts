...
export const createRepositoriesPlugin = async (mongoUri: string) => {
  const topicRepository = withTraceSync('questions.init.repositories.topic', () => new TopicRepository());

  const questionRepository = withTraceSync('questions.init.repositories.question', () => new QuestionRepository());
  ...
  await withTraceAsync('questions.init.repositories_plugin.mongoose', async (ctx) => {
  ...
  const plugin = withTraceSync('questions.init.repositories.plugin', () =>
    new Elysia({ name: 'Repositories' })
      ...
      .onStop(async () => {
        await withTraceAsync('questions.stop.repositories_plugin', async () => {
