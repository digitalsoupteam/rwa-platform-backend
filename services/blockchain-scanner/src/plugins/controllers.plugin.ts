...
export const createControllersPlugin = (servicesPlugin: ServicesPlugin) => {
  const getEventsCtrl = withTraceSync('blockchain-scanner.init.controllers.get_events', () =>
    getEventsController(servicesPlugin),
  );

  const getEventByIdCtrl = withTraceSync('blockchain-scanner.init.controllers.get_event_by_id', () =>
    getEventByIdController(servicesPlugin),
  );

  const plugin = withTraceSync('blockchain-scanner.init.controllers.plugin', () =>
    new Elysia({ name: 'Controllers' }).use(getEventsCtrl).use(getEventByIdCtrl),
  );
