...
describe("RWA Pool Trading", () => {
  ...
  test("should perform mint and burn operations", async () => {
    ...
    // Get and store initial pool state
    ...
    expect(initialPoolState.errors).toBeUndefined();
    
    const initialAwaitingRwaAmount = initialPoolState.data.getPool.awaitingRwaAmount;
    const initialRealHoldReserve = initialPoolState.data.getPool.realHoldReserve;
    const initialVirtualHoldReserve = initialPoolState.data.getPool.virtualHoldReserve;
    const initialVirtualRwaReserve = initialPoolState.data.getPool.virtualRwaReserve;

    console.log("Initial pool state:", {
      awaitingRwaAmount: initialAwaitingRwaAmount,
      realHoldReserve: initialRealHoldReserve,
      virtualHoldReserve: initialVirtualHoldReserve,
      virtualRwaReserve: initialVirtualRwaReserve
    });
    ...
    const poolStateAfterMint = await makeGraphQLRequest(
      GET_POOL,
      {
        id: poolData.poolId,
      },
    ...
    expect(poolStateAfterMint.errors).toBeUndefined();
    
    // Check that awaitingRwaAmount increased by rwaAmount
    expect(BigInt(poolStateAfterMint.data.getPool.awaitingRwaAmount)).toBe(
      BigInt(initialAwaitingRwaAmount) + BigInt(rwaAmount)
    );
    
    // Check that realHoldReserve increased
    expect(BigInt(poolStateAfterMint.data.getPool.realHoldReserve)).toBeGreaterThan(
      BigInt(initialRealHoldReserve)
    );

    // Perform burn operation
    const [holdAmountWithoutFee, holdFee, bonusAmountWithoutFee, bonusFee] = await poolContract.estimateBurn(rwaAmount);
    console.log("Burn estimation:", {
      holdAmountWithoutFee: holdAmountWithoutFee.toString(),
      holdFee: holdFee.toString(),
      bonusAmountWithoutFee: bonusAmountWithoutFee.toString(),
      bonusFee: bonusFee.toString()
    });
    ...
    expect(finalPoolState.errors).toBeUndefined();
    
    // Check that pool state returned to initial values after burn
    expect(finalPoolState.data.getPool.awaitingRwaAmount).toBe(initialAwaitingRwaAmount);
    expect(BigInt(finalPoolState.data.getPool.realHoldReserve)).toBe(BigInt(initialRealHoldReserve));
    expect(BigInt(finalPoolState.data.getPool.virtualHoldReserve)).toBe(BigInt(initialVirtualHoldReserve));
    expect(BigInt(finalPoolState.data.getPool.virtualRwaReserve)).toBe(BigInt(initialVirtualRwaReserve));
  });
});