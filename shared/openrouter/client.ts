...
export class OpenRouterClient {
  constructor(
    ...
    if (!this.apiKey) {
      throw new AppError({ message: "OpenRouter API key is not configured", statusCode: 500, code: "CONFIG_ERROR" });
    }
  }
  ...
  async completion(
    ...
    try {
      ...
      metrics.counter('openrouter_requests_total', { model, result: 'success', type: 'completion' });
      ...
      throw new AppError({ message: "Unknown error in OpenRouter completion request", statusCode: 502, code: "UPSTREAM_ERROR", cause: error });
    }
  }
  ...
  async chatCompletion(
    ...
    try {
      ...
      throw new AppError({ message: "Unknown error in OpenRouter chat completion request", statusCode: 502, code: "UPSTREAM_ERROR", cause: error });
    }
  }
  ...
  async getGeneration(
    ...
    try {
      ...
      throw new AppError({ message: "Unknown error fetching generation metadata", statusCode: 502, code: "UPSTREAM_ERROR", cause: error });
    }
  }
  ...
  async getModels(): Promise<OpenRouterModelsResponse> {
    try {
      ...
      throw new AppError({ message: "Unknown error fetching models list", statusCode: 502, code: "UPSTREAM_ERROR", cause: error });
    }
  }
}
